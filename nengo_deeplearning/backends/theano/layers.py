"""Neural network layers for Theano networks.
...
class Generic(object):
    """Layer that returns input unchanged.

    Useful when processing real valued vectors.
    ...
    weights : array_like, optional
        Used internally for saving/loading.
    """
...
class OneHot(object):
    def __init__(self, n_features, weights=None):
        self.settings = {'n_features': n_features,
                         'weights': weights}
    ...
    def output(self, dropout_active=False):
        ret = theano_one_hot(self.input.flatten(), self.n_features)
        return ret.reshape((self.input.shape[0],
                            self.input.shape[1],
                            self.size))


class SimpleRecurrent(object):
    def __init__(self, size=256, activation='tanh', init='orthogonal',
                 truncate_gradient=-1, seq_output=False, p_drop=0.,
                 weights=None):
        self.settings = {'size': size,
...
class LstmRecurrent(object):
    def __init__(self, size=256, activation='tanh',
                 gate_activation='steeper_sigmoid', init='orthogonal',
                 truncate_gradient=-1, seq_output=False, p_drop=0.,
                 weights=None):
        self.settings = {'size': size,
                         'activation': activation,
...
class GatedRecurrent(object):
    def __init__(self, size=256, activation='tanh',
                 gate_activation='steeper_sigmoid', init='orthogonal',
                 truncate_gradient=-1, seq_output=False, p_drop=0.,
                 direction='forward', weights=None):
        self.settings = {'size': size,
                         'activation': activation,
                         'gate_activation': gate_activation,
                         'init': init,
    ...
    def connect(self, l_in):
        ...
        self.params = [self.h0, self.w_z, self.w_r, self.w_h, self.u_z,
...
class Dense(object):
    def __init__(self, size=256, activation='rectify', init='orthogonal',
                 p_drop=0., weights=None):
        self.settings = {'size': size,
                         'activation': activation,
                         'init': init,
                         'p_drop': p_drop,
